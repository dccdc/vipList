<template>
	<v-container style="background-color: white;padding:0 ">
		<div style="position: relative;">
			<div style=" position: absolute;z-index: 100;background-color: white;width: 100%;">
				...
				<v-row style="height: 55px;padding:0 12px">
					...
					<v-spacer></v-spacer>
					<v-col class="pt-1" >
						<v-select style="width: 80px;float:right;font-size: 12px;" loader-height="20px" :items="items" v-model="selectVal" item-color="error"  color="#D81E06"  outlined dense ></v-select>
					</v-col>
				</v-row>
			</div>
		</div>

		<scroller ref="myscroller" style="padding-top: 110px;"  noDataText="我是有底线的" :on-refresh="refresh"
		 :on-infinite="infinite">
			<!-- content goes here -->
			<v-card flat>
				<v-card-text style="padding: 0 16px;">
					<v-list v-for="(item,index) in pointsList" :key="index" three-line>
							...
							<v-row>
								<v-col cols="8" style="padding: 0 12px;">
									<v-list-item-content>
										...
										<v-list-item-subtitle style="margin-top: 10px;font-size: 10px;">{{item.mer_name}}</v-list-item-subtitle>
										<v-list-item-subtitle style="margin-top: 10px;font-size: 10px;">{{timeFormate(item.point_date)}}</v-list-item-subtitle>
									</v-list-item-content>
								</v-col>
								<v-spacer></v-spacer>
								<v-col class="text-right" style="line-height: 90px;padding: 0 12px;">
